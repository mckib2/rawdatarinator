...
def readMeasDataVB15(filename):
    ...
    vals = [
        ...
        ('OSfactorPE','sKSpace.dPhaseOversamplingForDialog',1.0,lambda x:1.0+float(x)),
        ('NzAll','sKSpace.lPartitions',None,lambda x:int(x)),
        ('OSfactor3D','sKSpace.dSliceOversamplingForDialog',1.0,lambda x:1.0+float(x)),
        ...
        ('nAverage','lAverages',1,None),
        ('nRepetition','lRepetitions',1,lambda x:1+int(x)),
